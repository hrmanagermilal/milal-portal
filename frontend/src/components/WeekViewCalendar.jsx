...
function EventBars({ items, windowStart, windowEnd, onCellClick, roomId }) {
  ...
  return (
    <div 
      ...
      style={{ minHeight: `${dynamicHeight}px`, cursor: "pointer" }}
      onClick={() => onCellClick(roomId, windowStart)}
    >
      {placements.map(({ item, placement }, index) => (
          <div
            key={item.id}
            ...
            title={`${item.room_name} | ${placement.text} | ${item.requester_name}`}
          >
