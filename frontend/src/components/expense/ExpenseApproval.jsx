...
export default function ExpenseApproval({ initialExpenseId = null, onApprovalChanged }) {
  ...
  async function decide(action) {
    if (!selected || processing || !comment.trim() || (action === "approve" && !canApprove)) return;
      ...
      <Stack spacing={2.5}>
        ...
        <Paper elevation={0} sx={{ border: "1px solid #e0e6ef", borderRadius: "8px", minHeight: 360 }}>
          {selected ? <Stack spacing={2.5} sx={{ p: { xs: 2, sm: 3 } }}>
            ...
            {canDecide && <><Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
            ...
            <Stack direction="row" justifyContent="flex-end" spacing={1}><Button variant="outlined" color="error" startIcon={<CloseIcon />} disabled={processing || !comment.trim()} onClick={() => decide("reject")} sx={{ textTransform: "none", fontWeight: 700 }}>{korean ? "반려" : "Reject"}</Button><Button variant="contained" startIcon={<CheckIcon />} disabled={processing || !comment.trim() || !canApprove} onClick={() => decide("approve")} sx={{ textTransform: "none", fontWeight: 700, bgcolor: "#3b522e", "&:hover": { bgcolor: "#2f4325" } }}>{korean ? "결재 승인" : "Approve"}</Button></Stack></>}
