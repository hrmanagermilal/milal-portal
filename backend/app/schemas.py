...
class ExpenseAttachmentCreate(BaseModel):
    ...
    url: str = ""
    data_url: str = Field(default="", max_length=14_000_000)


class ReceiptExtractionRequest(BaseModel):
    file_data_url: str = Field(min_length=32, max_length=14_000_000)


class ReceiptExtractionOut(BaseModel):
    items: list[ExpenseItemCreate]
    hst_amount: float = Field(ge=0)
...
class ExpenseApprovalRouteUpdate(BaseModel):
    chairperson_member_id: int = Field(gt=0)
    finance_elder_member_id: int = Field(gt=0)


class ExpenseApprovalRouteOut(ExpenseApprovalRouteUpdate):
    id: int
    account_id: int
    account_code: str = ""
    account_name: str
